...
opts_knit$set(root.dir = "~/GitHub/Developing-Data-Products-Version-2/Murder_Shiny")
...
setwd("~/GitHub/Developing-Data-Products-Version-2/Murder_Shiny")
...
murders$City <- stri_trans_totitle(murders$City)
locations$City <- stri_trans_totitle(locations$City)
locations$County <- stri_trans_totitle(locations$County)
murders$City <- gsub('St.','St',murders$City)
locations$City <- gsub('St.','St',locations$City)
locations$County <- gsub('St.','St',locations$County)
murders$City <- gsub('Saint','St',murders$City)
locations$City <- gsub('Saint','St',locations$City)
locations$County <- gsub('Saint','St',locations$County)
murders$City <- gsub('\'','',murders$City)
locations$City <- gsub('\'','',locations$City)
locations$County <- gsub('\'','',locations$County)
murders$City <- gsub(' County','',murders$City)
locations$City <- gsub(' County','',locations$City)
locations$County <- gsub(' County','',locations$County)
murders$State <- substr(murders$Agency.Code,1,2)
murders[murders$State=="NB","State"] <- "NE"
...
locations[locations$Zip_code=="30366","County"] <- "De Kalb"
...
summarise(lat = mean(Latitude,na.rm =TRUE),lng = mean(Longitude,na.rm =TRUE))
colnames(latlong2)[1] <- "City"
latlng <- rbind(latlong,latlong2)
latlng <- latlng[!duplicated(latlng[,c("City","State")]),]
...
```{r}
 plotdata %>%  
  leaflet() %>%
  addTiles() %>%
  addMarkers(label=plotdata$title) %>%
  addCircles(weight = 1, radius = sqrt(plotdata$Victim.Count) * 500)
